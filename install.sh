...
case "$(lspci -d ::03xx)" in
    ...
    *[iI][nN][tT][eE][lL]*)
        ...
        ;;&
    *[nN][vV][iI][dD][iI][aA]*)
        echo "[1] NVIDIA kernel modules - module sources"
        echo "[2] NVIDIA open kernel modules - module sources"
        echo "[3] Open Source 3D acceleration driver for nVidia cards"
        echo "[q]"
...
echo "${P}" | passwd -R /mnt -s "${LOGIN}"

# Change password (root)
echo "${R}" | passwd -R /mnt -s
